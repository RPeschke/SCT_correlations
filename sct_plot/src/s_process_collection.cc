...
bool s_process_collection::process(FileProberties* fileP) {
  ...
  auto file = fileP->getTfile();

  m_plotCollection.reset();
  m_plotCollection = std::make_shared<S_plot_collection>(file);
  m_plotCollection->setOutputFile(m_dummy);

  m_output_planes = sct_plot::Create_Correlations_of_true_Fitted_hits_with_DUT_Hits_in_channels(*m_plotCollection,
                                                                                                m_add_gear->m_cuts,
                                                                                                m_add_gear->m_residual_cut,
                                                                                                *m_gear,
                                                                                                m_add_gear->m_rotation,
                                                                                                m_add_gear->m_pos_x,
                                                                                                s_plot_prob("Hits_in_channels").SaveToDisk());

  //m_output_planes = sct_plot::GBL_Create_Correlations_of_true_Fitted_hits_with_DUT_Hits(*m_plotCollection, S_YCut(-42,-36),m_residual_cut, *m_gear, m_rotation, m_pos_x ,s_plot_prob().SaveToDisk());
  m_res_VS_event.clear();

  auto res = sct_plot::residual(*m_plotCollection,
  ...
  double totalHits = (double)m_plotCollection->Draw(m_output_planes.getTotalTrueHits(),
                                                    S_DrawOption()
                                                    .cut_x(m_add_gear->m_active_area_x_min,
                                                    m_add_gear->m_active_area_x_max)
                                                    );
  ...
  //  std::cout << sqrt((DUTHits / totalHits)*(1 - (DUTHits / totalHits))*(1 / totalHits)) << std::endl;
...
Long64_t s_process_collection::DrawResidual() {
  ...
  auto ret = m_plotCollection->Draw(m_output_planes.getResidual(), S_DrawOption().draw_x());
...
Long64_t s_process_collection::DrawResidualVsEvent(Double_t min_X, Double_t max_X) {
  m_ResidualVsEvent = std::make_shared<TH2D>("ResidualVsEvent", "Residual Vs Event", 100, 0, 0, 100, min_X, max_X);
  return m_plotCollection->Draw(m_res_VS_event(), S_DrawOption().draw_x_VS_y().cut_x(min_X, max_X).output_object(m_resVSMissing.get()).opt_colz());
}

Long64_t s_process_collection::DrawResidualVsEvent() {

  auto ret = m_plotCollection->Draw(m_res_VS_event(), S_DrawOption().draw_x_VS_y());
...
Long64_t s_process_collection::DrawResidualVsMissingCordinate(Double_t min_X, Double_t max_X) {
  m_resVSMissing = std::make_shared<TH2D>("ResidualVsMissingCordinate", "Residual Vs Missing Cordinate", 100, 0, 0, 100, min_X, max_X);
...
Long64_t s_process_collection::DrawResidualVsMissingCordinate() {
  auto ret = m_plotCollection->Draw(m_output_planes.getResidualVSmissing(), S_DrawOption().draw_x_VS_y().opt_colz());
  TH2* h = dynamic_cast<TH2*>(gPad->GetPrimitive("htemp"));
  h->SetTitle("residual");
...
Long64_t s_process_collection::Draw_Efficinecy_map() {
  ...
  m_Efficieny_trueHits = std::make_shared<TH1D>("total", "total", m_add_gear->m_bins, -0.5, m_add_gear->m_numOfStrips - 0.5);

  m_plotCollection->Draw(m_output_planes.getTotalTrueHits(), S_DrawOption().draw_x().output_object(m_Efficieny_trueHits.get()));

  m_Efficieny_map = std::make_shared<TH1D>("Efficiency", "Efficiency", m_add_gear->m_bins, -0.5, m_add_gear->m_numOfStrips - 0.5);
...
Long64_t s_process_collection::Draw_Hit_map() {
  m_Hits_total = std::make_shared<TH1D>("total", "total", m_add_gear->m_bins, -0.5, m_add_gear->m_numOfStrips - 0.5);
...
Long64_t s_process_collection::Draw_DUT_Hits_map() {
  m_Hits_with_DUT_Hits = std::make_shared<TH1D>("DUT", "DUT", m_add_gear->m_bins, -0.5, m_add_gear->m_numOfStrips - 0.5);
...
void s_process_file::process_begin() {
  if (!m_add_gear) {
    m_add_gear = std::make_shared <sct_corr::additionalGear>();
  }
if (!m_outputl)
{
  m_outputl = std::make_shared <sct_corr::rootEventRunOutput>("out");
}
